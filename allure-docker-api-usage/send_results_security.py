...
def get_authenticated_session():
    ...
    response = session.post(login_uri, json=credentials_body, verify=ssl_verification)
    log_response(response)

    csrf_access_token = session.cookies['csrf_access_token']
    print("CSRF-ACCESS-TOKEN: " + csrf_access_token)
    session.headers['X-CSRF-TOKEN'] = csrf_access_token
    return session


def send_results_to_allure_server(session):
    print("------------------SEND-RESULTS------------------")
    request_body = {
    ...
    response = session.post(send_results_uri, params=uri_params, json=request_body, verify=ssl_verification)
    log_response(response)


def generate_allure_report(session):
    """
    ...
    response = session.get(generate_report_uri, params=uri_params, verify=ssl_verification)
